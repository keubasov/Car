class Parser
  ...
  ###################################################################
  ###   Выбирает пользователей, подписки которых совпадают с      ###
  ###       найденым  автомобилем и отправляет их список,         ###
  ###         вместе с самим объявлением в telegram               ###
  ###################################################################

  def search_overlap (ad, mes_queue)
    users_ids = Subscription.users_to_sub(ad)
    unless users_ids.blank?
      mes_queue<<[ad, users_ids]
    end
  end
  ...
  begin
    tds = tr.css('td')
    make, model =tds[2].text.strip.split(' ',3)[0,2]
